...
# Import CONFIG first to set up logging (INFO level initially based on config)
try:
    from src.config_loader import CONFIG # Triggers initial logging setup
    ...
    # Get logger AFTER initial setup
...
# --- ** NEW: Override Console Log Level for this Script ** ---
def suppress_console_info_logs():
    """Finds the console handler and sets its level to WARNING."""
    try:
        root_logger = logging.getLogger()
        found_console = False
        for handler in root_logger.handlers:
            # Check if it's a StreamHandler and not writing to a file (stderr might be used)
            if isinstance(handler, logging.StreamHandler) and not isinstance(handler, logging.FileHandler):
                 print(f"Updating level for Console Handler: {handler} to WARNING")
                 handler.setLevel(logging.WARNING)
                 found_console = True
                 break # Assume only one console handler
        if not found_console:
             print("WARNING: Could not find console handler to adjust level.")
    except Exception as e:
        print(f"WARNING: Failed to adjust console log level: {e}")

# Call the function to adjust level right after imports
suppress_console_info_logs()
# --- End of Log Level Override ---
...
def display_all_sections(city_name_internal, city_name_weather, city_name_aqicn):
    ...
    # NOTE: Logging within these called functions will respect the handler levels set above
    ...
    try:
        ...
        log.error(f"Error in Sec 0.5 for {city_name_internal}", exc_info=True) # Errors still log
    ...
    try:
        ...
        if hist_data_check is not None and not hist_data_check.empty:
            print(f"  (Data available for historical graph)") # Simplified output
        else:
            print("  (Historical data not available or city mismatch for graph)")
    except Exception as e:
    ...
    try:
        ...
        if current_aqi:
            ...
            color = aqi_info['color'] if aqi_info else '#808080'
...
if __name__ == "__main__":
    # Logging setup happens when config_loader is imported
    # Now override the console handler level
    suppress_console_info_logs() # Call the function defined above

    log.info("Starting end-to-end backend test script...") # This INFO won't show on console now

    # Get available cities list
    log.info("Attempting to get available cities...") # This INFO won't show
    available_cities = get_available_cities()
    if not available_cities:
        log.warning("Could not get cities from historical data, falling back to config.") # This WARNING will show
        ...
        if not available_cities:
             ...
             log.critical("No available cities found. Exiting test script.") # CRITICAL will show
             sys.exit(1)
        else:
             log.info(f"Using cities from config: {available_cities}") # This INFO won't show
    else:
         log.info(f"Using cities from historical data: {available_cities}") # This INFO won't show

    # Display choices and get input
    print("\nAvailable cities:") # Regular print always shows
    for i, city in enumerate(available_cities):
        print(f"  {i+1}. {city}")

    # (Keep the rest of the input loop and display_all_sections call as before)
    while True:
        try:
            ...
            if selected_city_internal:
                 ...
                 if selected_city_internal in CONFIG.get('modeling', {}).get('target_cities', []): # Check against config list
                      weather_api_query_city = f"{selected_city_internal}, India"
                 log.warning(f"Selected internal city: '{selected_city_internal}'") # Change to WARNING to see it
